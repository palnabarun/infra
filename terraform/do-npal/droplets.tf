...
resource "digitalocean_droplet" "jarvis" {
  image  = "ubuntu-18-04-x64"
  name   = "jarvis"
  region = "blr1"
  size   = "s-1vcpu-1gb"
  ...
  ipv6 = true
  ssh_keys = [digitalocean_ssh_key.nabarun_pal.id]
  tags = ["jarvis", "services", "blr"]
}

resource "digitalocean_floating_ip" "jarvis_fip" {
    droplet_id = digitalocean_droplet.jarvis.id
    region = digitalocean_droplet.jarvis.region
}
